...
class DependenceClass(StrEnum):
    ...
    than a missing one; the better point is that for a model
...
def classify_group(correlation_group: str, collector: str = "",
    ...
    discount is harder to notice than a missing one; the better point is that
    for a model whose dominant failure mode is overconfidence, unrecognised
    evidence should fail conservative.
    """
