...
# ---- tamper and truncation resistance ------------------------- #
...
def test_provenance_tampering_is_detected(tmp_path, field, value):
    """The entry hash covered only body identity, so it was possible to rewrite request
...
def test_suffix_truncation_is_detected(tmp_path):
    ...
    suffix. A review removed the final capture, decremented capture_count, left
...
# ---- verification must never execute package-supplied code ---------- #
